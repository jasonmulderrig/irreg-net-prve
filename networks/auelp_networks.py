...
def auelp_network_topology_initialization(
        network: str,
        date: str,
        batch: str,
        sample: int,
    ...
    the topology of an artificial uniform end-linked polymer network via
    a modified Gusev-Hanson protocol (which is a Monte Carlo procedure).

    Args:
        network (str): Lower-case acronym indicating the particular type of network that is being represented by the eventual network topology; here, only "auelp" is applicable (corresponding to artificial uniform end-linked polymer networks ("auelp")).
    ...
    r_mic = np.min(L)
    # Uniform polymer chain contour length
    l_cntr = nu * b
    ...
    # connected component
    mx_cmp_l_cntr_conn_edges = l_cntr * np.ones(np.shape(mx_cmp_conn_edges)[0])
...
def auelp_network_topology(
    ...
    artificial uniform end-linked polymer network. Then, the function
    calls the artificial uniform end-linked polymer network
    initialization function to create the artificial uniform end-linked
    polymer network.
    ...
    # artificial uniform end-linked polymer networks. Exit if a
    # different type of network is passed.
    if network != "auelp":
        error_str = (
            "Network topology initialization procedure is only "
            + "applicable for artificial uniform end-linked polymer "
